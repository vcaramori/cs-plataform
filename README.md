...
- **OAuth corrigido (PULL, opcional)** — endpoints reais confirmados por recon e gravados em [oauth.ts](src/lib/integrations/readai/oauth.ts): authorize `https://authn.read.ai/oauth2/auth` (302 → login), token `https://authn.read.ai/oauth2/token`, register `https://api.read.ai/oauth/register` (dynamic client registration **testado OK**). Descoberta agora tenta os `.well-known` de `authn.read.ai` e, em fallback, segue o **protected-resource metadata** (`.../.well-known/oauth-protected-resource/mcp` → `authorization_servers`). `audience` opcional via `READAI_OAUTH_AUDIENCE` (ative se `/v1/meetings` recusar o token por audience — confirmar no 1º login real).
- **Admin** (`/admin/settings` → aba **Read.ai**): novo card **Webhooks (recomendado)** com a URL pronta para copiar, campo de signing key(s) (rotação por linha) e CSM padrão — [ReadAiSettingsTab.tsx](src/app/(dashboard)/admin/settings/components/ReadAiSettingsTab.tsx) + ação `save_webhook` em [route](src/app/api/admin/readai-settings/route.ts).
...
- **Sem migração nova**: signing keys e CSM padrão ficam em `app_settings.readai_integration` (KV); colunas de `interactions` já existiam.
