...
> **Bugs de dados pré-existentes revelados ao destravar os crons** (separados do cron/auth):
> - `adoption-analysis`: a lógica consulta a tabela **`account_feature_adoption`**, que **não existe** (a tabela real é `feature_adoption`, com schema diferente — `status` em vez de `adoption_status`, sem `adoption_pct`). Falha em todas as contas; também **zera silenciosamente** o componente de adoção no `health-score`. Precisa de decisão sobre o modelo de dados de Adoção (não corrigido aqui para não alimentar dado errado no health score).
> - `auto-checkin/generate` e `proactive-alerts`: rodam (200) mas têm erros por-conta (ex.: "Bad escaped character in JSON" da IA) em algumas contas — degradação parcial, não bloqueio.
