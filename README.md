...
**Backfill resumível** (descoberto ao reprocessar): a API de lista com `expand[]` é **lenta**, então o histórico completo **não cabe** numa execução serverless (kill em 300s da Vercel) — e o estado só era salvo no fim, então cada ciclo recomeçava do topo e travava nas ~90 reuniões mais novas. Corrigido em [sync.ts](src/lib/integrations/readai/sync.ts): o **cursor de paginação é persistido a cada página**, o **orçamento de tempo é checado POR REUNIÃO** (~200s; cada reunião com transcrição leva ~30s p/ embeddar) e a execução devolve limpa antes do kill — o próximo ciclo **retoma** de onde parou.

**Aprendizados / limites do backfill histórico (2026-06-23):**
- **A transcrição só vem pelo token de quem participou/hospedou** a reunião. Confirmado via recurso único (`GET /v1/meetings/{id}?expand[]=transcript`): a transcrição existe e é recuperável (ex.: Suzano, 57 KB) — mas só via o token do CSM dono. Como hoje **poucos CSMs conectaram o Read.ai**, reuniões cujo dono não conectou **não terão transcrição** no histórico.
- **Overflow numérico corrigido**: durações/sentiment absurdos estouravam `numeric(5,2)`/`numeric(4,3)` e abortavam o ingest — agora `durationHours>24h` cai no default e o sentiment é clampado.
- **Extração de sinais por IA NÃO roda no backfill em massa**: rodava um LLM sobre cada transcrição inteira (~100s numa de 57 KB), estourava a função e **saturava o pool de conexões** (chegou a derrubar login/queries). Agora o `sync` passa `extractSignals:false`; a extração fica só na **ingestão em tempo real (webhook)**. Transcrição/resumo são salvos mesmo assim.
- **Conclusão — "de hoje para frente" (2026-06-23):** em vez de re-varrer o histórico (lento e de baixo retorno — reuniões antigas em geral não têm transcrição acessível pelos tokens conectados), o cron `readai-sync-hourly` foi religado em modo **forward-only**: todos os CSMs conectados ficaram com `historical_done=true` e `last_sync_at = início de hoje`, então o sync incremental só importa reuniões **de hoje em diante** (já com transcrição + sentimento via `expand[]`). O webhook também traz reuniões novas automaticamente. O modal "Detalhes do Esforço" mostra a transcrição quando ela existe.
...
- **Biblioteca de templates** por tipo (`onboarding_templates`/`_items`, `offset_days` calcula as datas); admin em `/onboarding/templates`. Seed: "Implantação Padrão (5+5)" e "Express (3+2)". Atividades **reordenáveis por arrastar-e-soltar** (alça de arraste; `framer-motion/Reorder`) — a ordem vira `sort_order` ao salvar.
...
| Sessão 27 Suporte — UX do detalhe + classificação + export | Detalhe do chamado: **scroll só na área de mensagens** (página não rola) com **fundo branco**; no sidebar direito, **SLA + Classificação fixos** no topo (só Responsável/Sentimento rolam). Nova **prioridade "Bug Blocker"** (`priority='blocker'`) e novo campo **"Detalhamento (bug/operação)"** (`support_tickets.resolution_notes`, auto-save onBlur) — migration `support_bug_blocker_and_resolution_notes`. **Exportação XLSX** passa a incluir **todas as colunas** de `support_tickets` (rótulos amigáveis + nome bruto nas demais) + `resolution_notes`. | ✅ Concluída 2026-06-24 |
