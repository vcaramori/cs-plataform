...
- **Conclusão**: o pipeline está correto e o webhook traz reuniões novas com transcrição automaticamente; o **backfill histórico em massa foi pausado** (cron `readai-sync-hourly` desligado durante os testes — reativar com `select cron.alter_job(job_id:=16, active:=true);`). O modal "Detalhes do Esforço" mostra a transcrição quando ela existe.
