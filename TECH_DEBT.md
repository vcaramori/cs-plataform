...
- [ ] **#18 — `AtividadesClient.tsx`** usa `db.rpc` (uma função) como **valor de filtro** em `.eq(...)` na aba de equipe — claramente incorreto; estava mascarado por `as any`. _Ação:_ definir o filtro correto (provável intenção: sem filtro de dono na visão de time).
- [ ] **#19 — `health-score-daily/route.ts`** ordena/filtra por `contract_status`, coluna que **não existe** em `accounts` (status fica em `contracts`). _Ação:_ corrigir a coluna/origem.
- [ ] **#20 — `weighted-score.ts`** faz embed de `adoption_metrics` que o PostgREST não reconhece (relação inexistente). _Ação:_ revisar a query de adoção.
