...
- [~] **#8 — ~302 `as any`** (padrão `const db = supabase as any`) anulam a tipagem e escondem bugs de coluna inexistente. _Parcial:_ `database.types.ts` já foi regenerado (destravou o #1), mas a contagem de `as any` até subiu (de 282 → 302) ao zerar os erros. _Ação restante:_ remover `as any` progressivamente, preferindo tipos reais às asserções.
