...
| Jun/2026 | **Painel de Suporte reconstruído**: redesenho do dashboard (`SupportDashboardClient`) em layout híbrido (executivo + operacional), cards uniformes por seção: Hero (CSAT, Compliance SLA, TMR útil, FCR); Operação agora (abertos, SLA vencido/atenção, a fechar); Velocidade & SLA (TMP/TMR/Resposta com corrido vs útil + barra de compliance); Volume & Tendência (recebidos/resolvidos/backlog/reabertos/interações + gráfico de área diário via novo `/api/support-dashboard/trend`); Distribuição (tabelas compactas top-8 de clientes e agentes). |
| Jun/2026 | **Indicadores de atendimento globais (área)**: o Painel Tático computa os indicadores via admin client (área-global) para usuários internos, em vez do escopo RLS do viewer — rotas `support-dashboard/{period,operational,by-agent,by-client}` (guarda: nega externo). Antes, CSAT/contas escopadas distorciam o "indicador da área". |
| Jun/2026 | **Anexos re-hospedados + SLA padrão + fix CSAT + UI**: (1) anexos/imagens inline dos chamados re-hospedados no bucket público `helpdesk-attachments` (migration `20260617160000`, helper `rehost.ts`) — não dependem mais do CDN externo; dedupe por sha256. (2) Métricas em horário comercial resolvem a política **por conta com fallback ao SLA Padrão** (`getPolicyForAccount`/`getBusinessHoursForAccount`). (3) Fix: CSAT não aparecia para usuário interno sem contas (super_admin) por RLS — nova policy `20260617180000` (interno lê todo CSAT); a avaliação já era importada corretamente. (4) UI: mensagem a 90% da largura; imagens de anexo (incl. respostas do agente) renderizadas inline + link. |
| Jun/2026 | **Conversa completa do chamado (thread HelpDesk) + tempo médio de resposta**: o detalhe (`TicketDetailClient`) renderiza a conversa importada como na ferramenta de origem — mensagens por ator (cliente/agente), logs de status/atribuição/avaliação, imagens inline e anexos, em scroll único. Nova tabela `helpdesk_thread_events` (migration `20260617140000`); `map.ts` (`buildThreadEvents`) reconstrói a thread dos `events` (mapeia `cid→url`, separa anexos), `sync.ts` persiste (apaga+reinsere). HTML sanitizado com `isomorphic-dompurify`; imagens/anexos do CDN público do HelpDesk. Novo indicador **Tempo Médio de Resposta** (solicitante→agente, corrido + útil) — colunas `avg_response_minutes`/`avg_response_business_minutes`. Re-importação: 305 chamados → 3.069 eventos. |
| Jun/2026 | **Indicadores: tempo útil (SLA) + interações + FCR**: o painel passou a mostrar TMP/TMR em **horário comercial** (`getBusinessMinutesBetween` + `business_hours` + timezone da política; pré-calc no sync em `first_response_business_minutes`/`resolution_business_minutes`) ao lado do tempo corrido; **média de interações** (mensagens públicas, `public_message_count`) por chamado resolvido; e **FCR** = % de encerrados com **1 resposta de agente** (`agent_reply_count`). Colunas via migration `20260617120000`. Interações/FCR ignoram chamados não processados (contagem 0 → "—") até reprocessar os históricos (resetar `historical_done` + Sincronizar). |
