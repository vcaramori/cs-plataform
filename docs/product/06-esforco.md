...
**Importação confiável (log + histórico + merge):** conectar o Read.ai (ou clicar "Importar minhas reuniões") dispara o backfill do próprio CSM **bounded por data** (piso `backfill_from`, default 1º de janeiro do ano corrente — configurável no banco) e o **cron horário completa sozinho** (resumível por cursor); o sistema conduz a importação de forma **autônoma e IO-safe**, sem varrer toda a história nem precisar de disparo manual. O backfill é **resumível**: como a API de lista com transcrição é lenta e o histórico não cabe numa execução serverless, o sync **persiste o cursor de paginação a cada página** e respeita um **orçamento de tempo (~240s)**, retomando de onde parou no ciclo seguinte (sem reprocessar o início). Toda reunião é registrada em **`readai_import_log`** e exposta no admin (card **"Histórico de importações"**: nova/mesclada/atualizada/pulada/erro/possível-duplicata + motivo). Botão **"Forçar histórico completo"** reimporta tudo. **Anti-duplicação**: se a reunião já foi lançada como esforço manual no mesmo dia/conta, ela **enriquece** esse esforço (transcrição/resumo/participantes) **preservando as horas lançadas à mão** — não duplica; ambíguo vira "possível duplicata" para revisão.

**Backfill com piso de data (`backfill_from`)** *(2026-06-23)*: o estado por CSM em `readai_sync_state` aceita um campo `backfill_from` (ex.: `2026-01-01`). Quando setado, o backfill aplica `start_time_ms.gte` **mesmo no modo histórico** → varre só reuniões ≥ essa data, do mais novo ao piso, **resumível por cursor** (bounded + retomável). Usado para importar um período específico (ex.: o histórico da Marcela de 01/01 em diante) **sem varrer toda a história nem pesar o I/O** — executado em lotes espaçados e monitorados (instância Supabase tem orçamento de Disk IO limitado; enriquecimento pesado só bounded/sequencial).
...
**OAuth (PULL):** o Read.ai não tem token estático — o acesso é **OAuth 2.1** (Authorization Code + PKCE, *dynamic client registration*, access token de ~10min com refresh **rotativo**) no authorization server **`authn.read.ai`** (ORY Hydra). As credenciais ficam **criptografadas** por CSM (`user_integrations`) e o token é renovado em background. Nada hardcoded. A leitura usa `GET /v1/meetings` com os campos enriquecidos pedidos via **`expand[]=`** (notação de array — `expand[]=transcript&expand[]=summary…`); a **transcrição** vem em `transcript.text` (texto completo "Nome: fala") ou, na falta, é montada de `transcript.turns[]`. O filtro incremental usa `start_time_ms.gte` (epoch ms).
...
| Jun/2026 | **Journal global + ordenação por data + fila de check-ins p/ Atividades**: o journal de /esforço aparecia vazio porque buscava por `logged_at` (data de import) enquanto filtra por `date` do evento — os imports históricos do Read.ai afundavam os lançamentos do período. Agora ordena por `date` desc (limit 500) e mostra **visão global** (RLS `time_entries_internal_view_all`; removido o filtro `csm_id`), com coluna **CSM**. A fila "Check-ins Pendentes de Aprovação" (`AutoCheckInQueue`) saiu de /esforço e foi para **/atividades** (é tarefa, não esforço). |
