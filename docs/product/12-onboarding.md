...
- **Biblioteca de templates** (`onboarding_templates` + `onboarding_template_items`): modelos por tipo (ex.: "Implantação Padrão 5+5 semanas", "Express 3+2"). Cada item tem `offset_days` (e `duration_days`) → o sistema **calcula as datas** dos marcos a partir da data de início. Admin em **/onboarding/templates**. As atividades do modelo são **reordenáveis por arrastar-e-soltar** (alça `GripVertical`, via `framer-motion/Reorder`); a nova ordem vira `sort_order` ao salvar.
...
- **Habilitar de dois jeitos**: botão **"Iniciar onboarding"** no painel `/onboarding` (escolhe um contrato ainda sem onboarding → template → início → responsável) **e** pelo painel do contrato na conta (auto-expandido quando ainda não iniciado).
- **Responsável (implantação, ≠ CSM)**: definido no início e **editável a qualquer momento** (`PATCH /api/onboarding`). Os candidatos a responsável são os usuários com **permissão de Onboarding (Editar) no perfil** — `GET /api/onboarding/owners`. Ou seja, "criar a permissão" = habilitar **Editar** no módulo **Onboarding** da matriz `/settings/roles` para o perfil do time de implantação.
