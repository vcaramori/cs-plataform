...
export function HealthScoreDetailsModal({ isOpen, onClose, accountId, accountName }: Props) {
  ...
  const groupedData = (data?.history || []).reduce((acc: any, h: any) => {
    ...
    // Se houver mais de um registro no mesmo dia, priorizamos os valores existentes
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto glass-card border-white/10 text-white">
        <DialogHeader>
          ...
          <div className="space-y-8 py-4">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between px-2">
                <h3 className="text-xs font-black uppercase tracking-widest text-slate-500">Evolução Temporal: Manual vs Shadow</h3>
            ...
            {/* History Table */}
            <div className="space-y-4">
              <h3 className="text-xs font-black uppercase tracking-widest text-slate-500 px-2">Histórico Completo de Eventos</h3>
              <div className="rounded-2xl border border-white/5 overflow-hidden">
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-white/5 border-b border-white/5">
                      <th className="px-4 py-3 font-black uppercase tracking-widest text-slate-500 text-[9px]">Referência</th>
                      <th className="px-4 py-3 font-black uppercase tracking-widest text-slate-500 text-[9px]">Tipo / Origem</th>
                      <th className="px-4 py-3 font-black uppercase tracking-widest text-slate-500 text-[9px] text-center">Score</th>
                      <th className="px-4 py-3 font-black uppercase tracking-widest text-slate-500 text-[9px]">Classificação</th>
                      <th className="px-4 py-3 font-black uppercase tracking-widest text-slate-500 text-[9px]">Nota / Raciocínio</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={h.id} className="hover:bg-white/[0.02] transition-colors group">
                        <td className="px-4 py-4">
                          <div className="flex flex-col">
                            <span className="font-bold text-white">{format(parseISO(h.date), 'dd/MM/yyyy')}</span>
                            <span className="text-[9px] text-slate-500 uppercase font-medium">Criado: {format(parseISO(h.created_at), 'dd/MM HH:mm')}</span>
                          </div>
                        </td>
                        <td className="px-4 py-4">
                          {h.manual_score !== null ? (
                            <div className="flex flex-col gap-1">
                              ...
                              <span className="text-slate-500 text-[9px] italic opacity-50">#manual</span>
                            </div>
                          ) : (
