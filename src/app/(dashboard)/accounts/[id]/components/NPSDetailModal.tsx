...
export function NPSDetailModal({ render, onOpenChange }: NPSDetailModalProps) {
  ...
  const segColor = seg === 'promoter' ? 'text-success bg-emerald-50 dark:bg-success/10 border-success-200 dark:border-success-500/20' : 
                   seg === 'passive' ? 'text-warning bg-amber-50 dark:bg-warning/10 border-warning-200 dark:border-warning-500/20' : 
      ...
      <DialogContent aria-describedby={undefined} className="bg-white dark:bg-slate-900 border border-border-divider dark:border-slate-800 max-w-xl p-0 overflow-hidden rounded-2xl shadow-2xl text-foreground dark:text-white">
        ...
        <div className="p-6 space-y-8">
          <div className="space-y-8">
            {/* Respondente e Nota */}
            <div className="flex items-center justify-between bg-surface-background dark:bg-slate-800/50 p-6 rounded-2xl border border-border-divider dark:border-slate-800">
              <div className="flex gap-4 items-center">
                <div className="w-12 h-12 rounded-2xl bg-white dark:bg-slate-900 border border-border-divider dark:border-slate-800 flex items-center justify-center shadow-sm">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-[10px] font-black text-content-secondary dark:text-content-secondary uppercase tracking-widest">Análise das Respostas</p>
              </div>
              ...
              <div className="space-y-4">
                {answers.length === 0 && !render.comment ? (
                  <div className="py-12 border-2 border-dashed border-border-divider dark:border-slate-800 rounded-2xl flex flex-col items-center justify-center bg-surface-background dark:bg-slate-800/50">
                    <p className="text-[10px] font-black text-content-secondary dark:text-content-secondary uppercase tracking-widest">Sem dados adicionais</p>
                  </div>
                ) : (
                  answers.map((ans: any) => (
                    <div key={ans.id} className="space-y-3 p-5 rounded-2xl bg-white dark:bg-slate-900 border border-border-divider dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition-all group shadow-sm">
                      ...
                      <div className="text-foreground dark:text-white text-sm font-bold tracking-tight leading-relaxed">
                        {ans.nps_questions?.type === 'nps_scale' ? (
                          <div className="flex items-center gap-4">
                            ...
                            <span className="text-indigo-500 font-black tracking-tighter">{ans.text_value}<span className="text-[10px] text-content-secondary ml-0.5">/10</span></span>
                          </div>
        ...
        <div className="p-6 bg-surface-background dark:bg-slate-800/50 border-t border-border-divider dark:border-slate-800 flex justify-end rounded-b-2xl">
          <Button 
            variant="outline" 
