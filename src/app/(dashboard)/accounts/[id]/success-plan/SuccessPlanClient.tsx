...
export function SuccessPlanClient() {
  ...
  return (
    <PageContainer className="space-y-6">
      ...
      <IndicatorDetailsModal isOpen={isDetailsOpen} onClose={() => setIsDetailsOpen(false)} indicator={currentSelectedIndicator} history={currentSelectedIndicator?.history || []} accountName={accountData?.name || 'Cliente'} onAddDataPoint={handleOpenEdit} onUpdated={() => queryClient.invalidateQueries({ queryKey: ['account-indicators', accountId] })} onDeleted={() => queryClient.invalidateQueries({ queryKey: ['account-indicators', accountId] })} />
