...
export function NPSFilters({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 bg-surface-card border border-border-divider px-3 py-2 rounded-2xl shadow-sm relative overflow-hidden">
      ...
      <Button variant="ghost" size="icon" onClick={onRefresh} className="h-8 w-8 text-content-secondary/40 hover:text-plannera-primary bg-surface-background/50 rounded-xl border border-border-divider shadow-sm transition-all active:scale-90">
        <RefreshCw className={`w-3.5 h-3.5 ${isLoading ? 'animate-spin' : ''}`} />
      </Button>

      <div className="w-px h-5 bg-border-divider/60 mx-1" />

      <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-plannera-orange/5 border border-plannera-orange/20 group transition-all hover:border-plannera-orange/40">
        <span className="text-[8px] font-black text-content-secondary/40 uppercase tracking-[0.25em]">Meta NPS</span>
        <span className="text-base font-black text-plannera-orange leading-none tracking-tighter tabular-nums">{goal ?? '—'}</span>
        <Dialog>
          <DialogTrigger asChild>
            <button className="p-1 rounded-lg bg-plannera-orange/10 hover:bg-plannera-orange text-plannera-orange hover:text-white transition-all active:scale-90">
          ...
          <DialogContent className="bg-surface-card border border-border-divider text-white max-w-sm rounded-2xl shadow-2xl p-0 overflow-hidden backdrop-blur-3xl">
            ...
            <div className="p-10 space-y-8">
              ...
              <Button onClick={onSetGoal} disabled={isUpdatingGoal} className="w-full h-14 bg-plannera-orange hover:bg-plannera-orange/90 text-white font-black text-[11px] tracking-[0.3em] rounded-2xl shadow-xl shadow-plannera-orange/20 transition-all active:scale-[0.98] uppercase">
      ...
      <Link href="/nps/programs">
        <Button className="h-8 bg-plannera-primary hover:bg-plannera-primary/90 text-white text-[10px] font-black uppercase tracking-[0.2em] px-4 rounded-xl shadow-xl shadow-plannera-primary/10 transition-all active:scale-95">
          Gestão
        </Button>
      </Link>
    </div>
