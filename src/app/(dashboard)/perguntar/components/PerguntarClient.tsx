...
export function PerguntarClient({ accounts }: { accounts: Account[] }) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-200px)] gap-6">
      {/* Scope Selector */}
      <div className="flex items-center gap-4 flex-wrap bg-surface-card border border-border-divider p-3 rounded-2xl shadow-sm">
        <div className="flex items-center gap-2 pl-2">
          <Target className="w-4 h-4 text-plannera-orange" />
          <span className="text-content-secondary text-[10px] font-bold uppercase tracking-widest">Escopo da I.A:</span>
        </div>
        <div className="w-72">
          <SearchableSelect
      ...
      {/* Main Container */}
      <Card className="flex-1 overflow-hidden flex flex-col relative shadow-xl">
        ...
        <CardContent className="flex-1 overflow-y-auto p-4 lg:p-10 space-y-10 relative z-10 scrollbar-thin scrollbar-thumb-white/5 scrollbar-track-transparent">
          <AnimatePresence mode="popLayout">
            {messages.length === 0 ? (
              ...
              >
                <div className="w-20 h-20 rounded-3xl bg-primary border border-primary/20 flex items-center justify-center relative group">
                ...
                <div className="text-center space-y-2">
                  <h2 className="text-2xl font-heading font-extrabold text-content-primary uppercase tracking-tight">Agente de Inteligência</h2>
                  <p className="text-content-secondary text-[10px] font-bold uppercase tracking-[0.3em] max-w-[400px] leading-relaxed opacity-70">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-2xl px-4">
                  {exampleQuestions.map((q, idx) => (
                    <motion.button
                      ...
                      onClick={() => setInput(q)}
                      className="group flex items-center justify-between text-left text-[10px] text-content-secondary hover:text-content-primary bg-surface-background hover:bg-surface-background/80 p-4 rounded-xl border border-border-divider transition-all shadow-sm"
                    >
              ...
              <div className="space-y-10">
                {messages.map((msg, idx) => (
                  ...
                  >
                    {msg.role === 'assistant' && (
                      <div className="w-10 h-10 rounded-xl bg-surface-background border border-border-divider flex items-center justify-center flex-shrink-0 mt-2 shadow-sm">
                    ...
                    <div className={cn(
                      ...
                      <div className={cn(
                        ...
                        msg.role === 'user'
                          ...
                          : msg.error
                            ? 'bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800/50 text-red-900 dark:text-white rounded-tl-sm'
                            : 'bg-surface-card border border-border-divider text-content-primary rounded-tl-sm'
                      )}>
                        {msg.role === 'assistant' ? (
                          <div className="prose prose-slate dark:prose-invert prose-sm max-w-none prose-headings:font-heading prose-headings:font-bold prose-headings:text-plannera-orange prose-strong:text-plannera-orange prose-p:text-content-primary">
                      ...
                      {msg.sources && msg.sources.length > 0 && (
                        ...
                        >
                          <div className="col-span-full flex items-center gap-2 mb-1">
                            ...
                            <span className="text-[10px] text-content-secondary font-bold uppercase tracking-widest">Grounding / Evidências Corporativas</span>
                          </div>
                          {msg.sources.slice(0, 4).map((s, si) => (
                            <div key={si} className="group/src flex items-start gap-4 bg-surface-background border border-border-divider rounded-xl px-4 py-3 hover:bg-surface-background/80 transition-all cursor-default relative overflow-hidden shadow-sm">
                              ...
                              <div className="min-w-0 flex-1 relative z-10">
                                <p className="text-content-primary text-[11px] font-bold uppercase tracking-tight truncate mb-1 group-hover/src:text-plannera-orange transition-colors">{s.title}</p>
                                <div className="flex items-center gap-3 text-[9px] font-bold uppercase tracking-widest text-content-secondary">
          ...
          {isLoading && (
            ...
            >
              <div className="w-10 h-10 rounded-xl bg-plannera-sop border border-border-divider flex items-center justify-center flex-shrink-0 shadow-lg animate-pulse">
                <Sparkles className="w-5 h-5 text-plannera-orange" />
              </div>
              <div className="bg-surface-card border border-border-divider rounded-2xl rounded-tl-sm px-6 py-4">
        ...
        {/* Input Area */}
        <div className="p-6 lg:p-10 border-t border-border-divider bg-surface-background relative z-20">
          <div className="max-w-4xl mx-auto relative group">
            ...
            <div className="relative flex items-end gap-2 bg-surface-card border border-border-divider rounded-2xl p-2 transition-all focus-within:border-plannera-orange/50 shadow-xl">
              <Textarea
                value={input}
                ...
                rows={1}
                disabled={isLoading}
                className="flex-1 bg-transparent border-none text-content-primary placeholder:text-content-secondary/40 resize-none px-4 py-3 min-h-[56px] focus-visible:ring-0 focus-visible:ring-offset-0 font-bold tracking-tight text-base"
          ...
          <div className="flex items-center justify-center gap-6 mt-4 opacity-70">
            <div className="flex items-center gap-1.5">
              <span className="text-[8px] font-bold uppercase text-content-secondary tracking-[0.2em]">Contexto: {selectedAccountId === 'all' ? 'Portfólio Global' : selectedAccount?.name}</span>
            </div>
            <p className="text-content-secondary text-[9px] font-bold uppercase tracking-widest hidden lg:block">
