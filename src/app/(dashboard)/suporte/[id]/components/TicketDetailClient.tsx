...
const statusCfg: Record<string, { label: string; color: string; dot: string }> = {
  open:        { label: 'Aberto',       color: 'text-red-400',     dot: 'bg-red-400' },
  in_progress: { label: 'Em Andamento', color: 'text-amber-400',   dot: 'bg-amber-400' },
  reopened:    { label: 'Reaberto',     color: 'text-orange-400',  dot: 'bg-orange-400' },
  resolved:    { label: 'Resolvido',    color: 'text-emerald-400', dot: 'bg-emerald-400' },
  closed:      { label: 'Fechado',      color: 'text-slate-400',   dot: 'bg-slate-400' },
}
...
const eventMeta: Record<string, { icon: React.ElementType; label: string; color: string }> = {
  ...
  ticket_closed:      { icon: CheckCircle2, label: 'Ticket fechado',           color: 'text-slate-400' },
...
function ClientMessage({ text, ts }: { text: string; ts: string }) {
  return (
    <div className="flex gap-3">
      <div className="w-8 h-8 rounded-full bg-slate-700 border border-white/10 flex items-center justify-center shrink-0 mt-0.5">
        <User className="w-4 h-4 text-slate-300" />
      </div>
      <div className="flex-1 max-w-2xl">
        <div className="flex items-center gap-2 mb-1.5">
          <span className="text-white text-xs font-semibold">Cliente</span>
          <span className="text-slate-500 text-[11px]">{fmtTs(ts)}</span>
        </div>
        <div className="bg-slate-800/80 border border-white/5 rounded-2xl rounded-tl-sm p-4">
  ...
  return (
    <div className="flex gap-3 flex-row-reverse">
      ...
      <div className="flex-1 max-w-2xl flex flex-col items-end">
        <div className="flex items-center gap-2 mb-1.5">
          <span className="text-slate-500 text-[11px]">{fmtTs(event.occurred_at)}</span>
          <span className="text-indigo-400 text-xs font-semibold">{shortName}</span>
        </div>
        <div className="bg-indigo-600/10 border border-indigo-500/20 rounded-2xl rounded-tr-sm p-4 w-full text-right">
          <p className="text-slate-200 text-sm leading-relaxed whitespace-pre-wrap text-left">{event.metadata?.body}</p>
        </div>
      </div>
    </div>
  )
...
function InternalNote({ event }: { event: SLAEvent }) {
  ...
  return (
    <div className="flex gap-3">
      <div className="w-8 h-8 rounded-full bg-amber-500/20 border border-amber-500/20 flex items-center justify-center shrink-0 mt-0.5">
        <Lock className="w-3.5 h-3.5 text-amber-400" />
      </div>
      <div className="flex-1 max-w-2xl">
        <div className="flex items-center gap-2 mb-1.5">
          <span className="text-amber-400 text-xs font-semibold">{shortName}</span>
          <span className="bg-amber-500/10 border border-amber-500/20 text-amber-500 text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full flex items-center gap-1">
            <Lock className="w-2.5 h-2.5" /> Nota Interna
          </span>
          <span className="text-slate-500 text-[11px] ml-auto">{fmtTs(event.occurred_at)}</span>
        </div>
        <div className="bg-amber-500/5 border border-amber-500/15 rounded-2xl rounded-tl-sm p-4">
          <p className="text-amber-100/80 text-sm leading-relaxed whitespace-pre-wrap">{event.metadata?.body}</p>
        </div>
      </div>
    </div>
  )
...
function LifecycleDivider({ event }: { event: SLAEvent }) {
  const meta = eventMeta[event.event_type] ?? { icon: CheckCircle2, label: event.event_type, color: 'text-slate-500' }
  ...
  return (
    <div className="flex items-center gap-3 py-1">
      ...
      <div className={cn('flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-widest shrink-0', meta.color)}>
        ...
        {meta.label}
        <span className="text-slate-600 font-normal normal-case">· {fmtTs(event.occurred_at)}</span>
      </div>
      <div className="h-px flex-1 bg-white/5" />
    </div>
  )
...
export function TicketDetailClient({ ticket: init, events: initEvents, messages: initMessages, csatResponse, agents, currentUserId }: Props) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* ── Header ───────────────────────────────────────────────────── */}
      <div className="border-b border-white/5 bg-slate-950/60 backdrop-blur px-6 py-4 shrink-0">
        {/* Breadcrumb */}
        <div className="flex items-center gap-1.5 text-slate-500 text-[11px] font-semibold uppercase tracking-widest mb-3">
          <Link href="/suporte" className="hover:text-white transition-colors flex items-center gap-1">
            <ArrowLeft className="w-3 h-3" /> Suporte
          </Link>
          <ChevronRight className="w-3 h-3" />
          {ticket.external_ticket_id && (
            <>
              <span className="text-slate-600 font-mono">#{ticket.external_ticket_id}</span>
              <ChevronRight className="w-3 h-3" />
            </>
          )}
          <span className="text-slate-400 truncate max-w-md">{ticket.title}</span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="space-y-1.5 flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 text-xs text-slate-500">
              <Link href={`/accounts/${account.id}`} className="flex items-center gap-1.5 text-indigo-400 hover:text-indigo-300 transition-colors font-medium">
              ...
              {ticket.parent_ticket_id && (
                <>
                  <span>·</span>
                  <Link href={`/suporte/${ticket.parent_ticket_id}`} className="flex items-center gap-1 hover:text-white transition-colors">
      ...
      {/* ── SLA Warning Banner ───────────────────────────────────────── */}
      {!hasSLA && (
        <div className="bg-amber-500/8 border-b border-amber-500/20 px-6 py-2.5 flex items-center justify-between shrink-0">
          ...
          <Link
            ...
            className="flex items-center gap-1 text-amber-400 hover:text-amber-300 text-[11px] font-bold uppercase tracking-widest transition-colors whitespace-nowrap ml-4"
          >
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex flex-col flex-1 min-w-0 border-r border-white/5">
          ...
          {/* Thread (scrollable) */}
          <div ref={threadRef} className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            {ticket.thread_content && (
              <div className="flex gap-3">
                ...
                <div className="flex-1 max-w-2xl">
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="text-slate-400 text-xs font-semibold">Thread de E-mail</span>
                  </div>
                  <div className="bg-slate-800/40 border border-white/5 rounded-2xl rounded-tl-sm p-4 max-h-48 overflow-y-auto">
                    <p className="text-slate-400 text-xs leading-relaxed whitespace-pre-wrap font-mono">{ticket.thread_content}</p>
                  </div>
                </div>
              </div>
            )}
          ...
          {/* Compose area ─────────────────────────────────────────── */}
          <div className="border-t border-white/5 bg-slate-950/40 p-4 shrink-0">
            {/* Tabs */}
            <div className="flex items-center gap-1 mb-3 bg-black/20 p-1 rounded-xl border border-white/5 w-fit">
              <button
              ...
              <button
                ...
                className={cn(
                  'flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-[11px] font-bold uppercase tracking-widest transition-all',
                  tab === 'note'
                    ? 'bg-amber-500/20 text-amber-400 shadow border border-amber-500/30'
                    : 'text-slate-500 hover:text-white'
                )}
              >
            ...
            <Textarea
              ...
              rows={4}
              className={cn(
                'bg-black/30 text-slate-200 placeholder:text-slate-600 text-sm rounded-xl resize-none border transition-colors',
                tab === 'reply' ? 'border-indigo-500/20 focus:border-indigo-500/40' : 'border-amber-500/10 focus:border-amber-500/30'
              )}
            />
              ...
              {tab === 'reply' ? (
                reviewApproved ? (
                  <Select value={outcome} onValueChange={(v: any) => setOutcome(v)}>
                    <SelectTrigger className="w-[190px] h-9 bg-black/20 border-white/5 text-[11px] font-bold uppercase tracking-widest">
                    ...
                    <SelectContent className="bg-slate-900 border-white/10">
                      ...
                      <SelectItem value="solution"        className="text-[11px] font-bold uppercase text-emerald-400">Resolver Chamado</SelectItem>
                      <SelectItem value="pending_client"  className="text-[11px] font-bold uppercase text-amber-400">Aguardar Cliente</SelectItem>
                      <SelectItem value="pending_product" className="text-[11px] font-bold uppercase text-indigo-400">Aguardar Produto</SelectItem>
                    </SelectContent>
                  </Select>
                ) : (
                  <span className="text-[10px] text-slate-600 italic">Avalie a resposta para continuar</span>
                )
              ) : <div />}
              ...
              <div className="flex items-center gap-2">
                {tab === 'reply' && reviewApproved && (
                  <Button
                    ...
                    size="sm"
                    variant="outline"
                    className="font-bold uppercase tracking-widest text-[10px] gap-1.5 border-white/10 text-slate-400 hover:text-white hover:border-white/20"
                  >
        ...
        {/* RIGHT: Sidebar ──────────────────────────────────────────── */}
        <div className="w-80 shrink-0 overflow-y-auto bg-slate-950/30">

          {/* ─ SLA ───────────────────────────────────────────── */}
          <section className="border-b border-white/5 p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em]">SLA</p>
                {!hasSLA && <AlertTriangle className="w-3 h-3 text-amber-400" />}
              </div>
              <Link href={`/accounts/${account.id}/sla`} className="flex items-center gap-1 text-[10px] text-slate-500 hover:text-indigo-400 transition-colors font-semibold uppercase tracking-widest">
            ...
            {!hasSLA ? null : (
              <div className="space-y-3">
                <div>
                  <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest mb-1">1ª Resposta</p>
                  <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest mb-1">Resolução</p>
                  <div className="flex items-center gap-2">
          ...
          <section className="border-b border-white/5 p-4 space-y-3">
            <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em]">Classificação</p>

            <div className="space-y-2.5">
              <div>
                <label className="text-[10px] text-slate-600 font-bold uppercase tracking-widest block mb-1">Status</label>
                <Select value={editStatus} onValueChange={setEditStatus}>
                  <SelectTrigger className="w-full bg-black/30 border-white/8 text-slate-300 text-xs h-9">
                    <SelectValue placeholder="Status" />
                  </SelectTrigger>
                  <SelectContent className="bg-slate-900 border-white/10 text-slate-300">
                    {STATUSES.map(s => (
                      <SelectItem key={s.value} value={s.value} className="text-xs hover:bg-white/5 focus:bg-white/10 transition-colors">
                        {s.label}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div>
                <label className="text-[10px] text-slate-600 font-bold uppercase tracking-widest block mb-1">Prioridade</label>
                <Select value={editPriority} onValueChange={setEditPriority}>
                  ...
                  <SelectContent className="bg-slate-900 border-white/10 text-slate-300">
                    {PRIORITIES.map(p => (
                      <SelectItem key={p.value} value={p.value} className="text-xs hover:bg-white/5 focus:bg-white/10 transition-colors">
                        {p.label}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div>
                <label className="text-[10px] text-slate-600 font-bold uppercase tracking-widest block mb-1">Nível Interno (SLA)</label>
                <Select value={editLevel} onValueChange={setEditLevel}>
                  ...
                  <SelectContent className="bg-slate-900 border-white/10 text-slate-300">
                    ...
                    {LEVELS.map(l => (
                      <SelectItem key={l.value} value={l.value} className="text-xs hover:bg-white/5 focus:bg-white/10 transition-colors">
                        {l.label}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div>
                <label className="text-[10px] text-slate-600 font-bold uppercase tracking-widest block mb-1">Categoria / Tópico</label>
                <Select value={editCategory} onValueChange={setEditCategory}>
                  ...
                  <SelectContent className="bg-slate-900 border-white/10 text-slate-300">
                    ...
                    {CATEGORIES.map(c => (
                      <SelectItem key={c} value={c} className="text-xs hover:bg-white/5 focus:bg-white/10 transition-colors">
                        {c}
                      </SelectItem>
                    ))}
                  </SelectContent>
          ...
          {/* ─ Responsável ───────────────────────────────────── */}
          <section className="border-b border-white/5 p-4 space-y-3">
            <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em]">Responsável</p>

            <div className="flex items-center gap-2.5 p-2.5 bg-white/[0.03] rounded-lg border border-white/5">
              ...
              <div>
                <p className="text-white text-xs font-semibold">{currentAgentEmail?.split('@')[0] ?? 'Não atribuído'}</p>
                <p className="text-slate-600 text-[10px]">{currentAgentEmail ?? '—'}</p>
              </div>
            </div>
            ...
            {agents.length > 0 && (
              <div className="space-y-2">
                ...
                <Button
                  ...
                  size="sm"
                  className="w-full bg-slate-800 hover:bg-slate-700 text-white border border-white/10 font-bold uppercase tracking-widest text-[10px] gap-1.5"
                >
          ...
          {/* ─ Info do Cliente ───────────────────────────────── */}
          <section className="border-b border-white/5 p-4 space-y-3">
            <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em]">Cliente</p>
            <div className="flex items-start gap-2.5">
              ...
              <div>
                <p className="text-white text-xs font-semibold">{account.name}</p>
                <Link
          ...
          <section className="p-4 space-y-2.5">
            <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] mb-3">Histórico</p>
            {[
              { label: 'Aberto em',      value: ticket.opened_at ? fmtTs(ticket.opened_at + 'T12:00:00') : null, color: 'text-slate-300' },
              { label: '1ª Resposta',    value: ticket.first_response_at ? fmtTs(ticket.first_response_at) : null, color: 'text-emerald-400' },
              { label: 'Resolvido em',   value: ticket.resolved_at ? fmtTs(ticket.resolved_at) : null, color: 'text-emerald-400' },
              { label: 'Fechado em',     value: ticket.closed_at ? fmtTs(ticket.closed_at) : null, color: 'text-slate-400' },
            ].map(({ label, value, color }) => (
              <div key={label} className="flex items-start justify-between gap-2">
                <span className="text-[10px] text-slate-600 font-bold uppercase tracking-widest shrink-0">{label}</span>
                <span className={cn('text-[11px] font-mono text-right', value ? color : 'text-slate-700')}>
                  {value ?? '—'}
                </span>
              </div>
            ))}
