    ...
    <PageContainer className="max-w-6xl space-y-10">
      ...
      <Tabs defaultValue="users" className="w-full space-y-6">
        <TabsList className="bg-surface-card p-1 rounded-2xl h-12">
          ...
          <TabsTrigger value="roles" className="rounded-xl px-6 py-2.5 text-xs font-bold uppercase tracking-wider data-[state=active]:bg-white dark:data-[state=active]:bg-slate-800 transition-all shadow-sm">
            Perfis de Acesso
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="roles" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1">
              <Card className="relative overflow-hidden border border-border-divider shadow-md bg-surface-card">
                ...
                <CardHeader className="pb-6">
                  ...
                  <CardDescription className="text-[10px] text-content-secondary uppercase font-bold tracking-wider opacity-60">
                    Definir papéis personalizados dinâmicos
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <form onSubmit={onSubmitRole} className="space-y-5">
                    <div className="space-y-1.5">
                      <Label className="text-[10px] font-bold text-content-primary ml-1 uppercase tracking-widest">Nome do Perfil *</Label>
                      <Input
                        type="text"
                        value={roleName}
                        onChange={(e) => setRoleName(e.target.value)}
                        placeholder="Ex: Auditor Pleno, Terceirizado"
                        className="transition-all focus:border-plannera-orange h-10 text-xs font-medium bg-surface-background rounded-xl"
                        required
                      />
                    </div>
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-[10px] font-bold text-content-primary ml-1 uppercase tracking-widest">Descrição do Perfil</Label>
                      <Input
                        type="text"
                    ...
                    <Button 
                      ...
                      className="w-full bg-primary hover:bg-primary/90 text-white font-bold uppercase tracking-widest h-11 rounded-xl shadow-lg transition-all active:scale-95 gap-2 text-xs"
                    >
                      {creatingRole ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                      {creatingRole ? 'Criando...' : 'Adicionar Perfil'}
                    </Button>
                  </form>
                </CardContent>
              </Card>
            ...
            <div className="lg:col-span-2">
              <Card className="border border-border-divider shadow-md bg-surface-card h-full">
                <CardHeader className="pb-6">
                  <CardTitle className="text-sm font-bold uppercase tracking-widest text-content-primary">Perfis Cadastrados</CardTitle>
                </CardHeader>
                <CardContent>
                  {loadingRoles ? (
                    <div className="flex flex-col items-center justify-center p-20 gap-4">
                      <Loader2 className="w-10 h-10 text-primary animate-spin" />
                      <span className="text-[10px] text-content-secondary font-bold uppercase tracking-[0.2em] animate-pulse">Carregando Perfis...</span>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      <AnimatePresence mode='popLayout'>
                        {roles.length === 0 ? (
                          <p className="text-content-secondary/40 text-[10px] font-extrabold uppercase tracking-widest text-center py-20">Nenhum perfil customizado cadastrado.</p>
                        ) : (
                          roles.map((role, idx) => (
                            <motion.div
                              key={role.id}
                              ...
                              animate={{ opacity: 1, x: 0 }}
                              transition={{ delay: idx * 0.05 }}
                              className="group flex flex-col p-4 rounded-xl border border-border-divider bg-surface-background hover:bg-surface-background/80 transition-all shadow-sm gap-2"
                            >
                              {editingRoleId === role.id ? (
                                <div className="space-y-3">
                                  <div className="flex gap-2">
                                    <Input
                                      ...
                                      className="h-9 text-xs font-bold rounded-lg bg-surface-card"
                                      placeholder="Nome do Perfil"
                                    />
                                    <Button
                                      size="sm"
                                      className="bg-emerald-600 hover:bg-emerald-700 h-9 px-3 rounded-lg text-white"
                                      onClick={() => onSaveEditedRole(role.id)}
                                      disabled={savingRole}
                                    >
                                      {savingRole ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
                                    </Button>
                                    <Button
                                      size="sm"
                                      variant="ghost"
                                      className="h-9 w-9 p-0 hover:bg-surface-card rounded-lg"
                                  ...
                                  <Input
                                    ...
                                    className="h-9 text-xs rounded-lg bg-surface-card"
                                ...
                                <div className="flex items-center justify-between">
                                  <div>
                                    <div className="flex items-center gap-2">
                                      <span className="text-xs font-extrabold uppercase tracking-widest bg-primary/10 text-primary px-2.5 py-1 rounded-lg">
                                        {role.name}
                                      </span>
                                    </div>
                                    <p className="text-content-secondary text-[10px] font-semibold mt-2.5 leading-relaxed">
                                      {role.description || 'Sem descrição cadastrada para este perfil.'}
                                    </p>
                                  </div>
                                  ...
                                  <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <Button
                                      ...
                                      className="h-8 w-8 p-0 hover:bg-surface-card rounded-lg"
                                      title="Editar Perfil"
                                    >
                                      <Edit2 className="w-3.5 h-3.5 text-content-secondary" />
                                    </Button>
                                    
                                    {/* Don't allow deleting core system profiles if necessary, but keep it open since they requested dynamic roles */}
                                    <Button
                                      size="sm"
                                      variant="ghost"
                                      onClick={() => onDeleteRole(role.id, role.name)}
                                      className="h-8 w-8 p-0 hover:bg-red-50 hover:text-red-600 rounded-lg text-content-secondary"
