          ...
          <div key={step.key} className="flex items-center">
            <div className="flex flex-col items-center gap-1">
              <div className={cn(
                ...
                isDone ? 'bg-success border-success-500' :
                isCurrent ? (isReopened ? 'bg-warning border-warning-500' : 'bg-indigo-400 border-indigo-400') :
            ...
            {i < STEPS.length - 1 && (
              <div className={cn(
                'h-px w-8 mb-3 mx-1',
                i < currentIndex ? 'bg-success' : 'bg-slate-700'
              )} />
            )}
          </div>
        )
