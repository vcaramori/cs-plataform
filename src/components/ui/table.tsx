...
>(({ className, ...props }, ref) => (
  <tfoot
    ref={ref}
    className={cn(
      "border-t bg-surface-background dark:bg-slate-800/50 font-medium [&>tr]:last:border-b-0",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <tr
    ref={ref}
    className={cn(
      "border-b border-border-divider transition-all hover:bg-muted/40 group cursor-pointer data-[state=selected]:bg-surface-card dark:data-[state=selected]:bg-slate-800",
      className
    )}
    {...props}
  />
...
>(({ className, sortKey, currentSort, onSort, children, ...props }, ref) => {
  ...
  return (
    <th
      ref={ref}
      className={cn(
        "h-12 px-4 text-left align-middle font-extrabold text-content-secondary [&:has([role=checkbox])]:pr-0 uppercase tracking-widest text-[11px] cursor-pointer hover:bg-surface-card transition-colors select-none group",
        className
      )}
    ...
    >
      <div className="flex items-center gap-1">
        {children}
        <span className="inline-flex items-center justify-center w-4 h-4 rounded hover:bg-accent/10">
