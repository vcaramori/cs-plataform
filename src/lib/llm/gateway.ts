 ...
 */
async function geminiGenerate(prompt: string, modelId: string): Promise<string> {
  try {
    ...
    // Tratamento de erro 503 (Overloaded) simplificado para o novo SDK
...
export async function generateText(
  ...
  try {
    const text = await geminiGenerate(prompt, modelId)
    return { 
      result: text, 
      provider: 'gemini', 
