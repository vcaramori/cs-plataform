...
CREATE TABLE IF NOT EXISTS account_risk_assessments (
...
CREATE INDEX IF NOT EXISTS idx_account_risk_assessments_account_id_analyzed_at ON account_risk_assessments(account_id, analyzed_at DESC);

-- RLS
ALTER TABLE account_risk_assessments ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Risk assessments are visible to users of the same company" ON account_risk_assessments;
CREATE POLICY "Risk assessments are visible to users of the same company" ON account_risk_assessments
  FOR SELECT USING (
    auth.role() = 'authenticated'
  );

DROP POLICY IF EXISTS "Risk assessments can be inserted by authenticated users (webhooks)" ON account_risk_assessments;
